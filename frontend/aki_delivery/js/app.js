...
function renderMenu() {
    ...
    <article class="product-card" data-product-id="${product.id}" tabindex="0" role="button" aria-label="Ver detalhes de ${product.name}">
      ...
      <div class="product-body">
        ...
        <div class="product-bottom">
          ...
          <button class="add" type="button" onclick="event.stopPropagation(); addToCart(${product.id})" aria-label="Adicionar ${product.name}">
  ...
  target.querySelectorAll('.product-card').forEach(card => {
    const open = () => openProductModal(Number(card.dataset.productId));
...
function openProductModal(id) {
  ...
  document.querySelector('#modalProductPrice').textContent = money(product.price);
  document.querySelector('#modalProductDescription').textContent = product.description;
  const tag = document.querySelector('#modalProductTag');
  tag.hidden = !product.tag;
  tag.textContent = product.tag || '';
  document.querySelector('#modalQty').textContent = modalQuantity;
  modal.hidden = false;
...
function addModalProductToCart() {
  ...
  for (let i = 0; i < modalQuantity; i += 1) addToCart(modalProduct.id);
  closeProductModal();
}
...
function initShared() {
  ...
  document.querySelector('#modalQtyMinus')?.addEventListener('click', () => { modalQuantity = Math.max(1, modalQuantity - 1); document.querySelector('#modalQty').textContent = modalQuantity; });
  document.querySelector('#modalQtyPlus')?.addEventListener('click', () => { modalQuantity += 1; document.querySelector('#modalQty').textContent = modalQuantity; });
  document.querySelector('#modalAddButton')?.addEventListener('click', addModalProductToCart);
